// Nav-Icons aus src/lib/svgs/MAIN, konvertiert zu React-Components mit
// currentColor stroke. Außenfarbe wird via `style={{ color }}` des Wrapper
// gesetzt — Active/Idle-State steuert nur den Color-Wert.

export function HomeNavIcon({ size = 24, active = false }: { size?: number; active?: boolean }) {
  ...
  return (
    ...
    >
      <path
        ...
        strokeLinecap="round"
      />
      <path d="M15 18H9" stroke="currentColor" strokeWidth={sw} strokeLinecap="round" />
    </svg>
  );
}

export function LibraryNavIcon({ size = 24, active = false }: { size?: number; active?: boolean }) {
  const sw = active ? 2 : 1.6;
  return (
    <svg
      width={size}
...
export function PenNavIcon({ size = 24, active = false }: { size?: number; active?: boolean }) {
  const sw = active ? 2 : 1.6;
  return (
    <svg
      width={size}
